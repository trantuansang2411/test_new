...
describe("Products", () => {
  ...
  // Helper: tạo order phục vụ GET /:id
  async function createTestOrder(app, token, productId) {
    let pid = productId;
    if (!pid) {
      const product = {
        name: `Product for order ${Date.now()}`,
        description: "Auto-created for GET /:id test",
        price: 10,
      };
      const r = await chai
        .request(app.app)
    ...
    const orderData = [{ _id: pid, quantity: 1 }];
    const res = await chai
      .request(app.app)
      .post("/buy")
      .set("Authorization", `Bearer ${token}`)
  ...
  // Helper: poll đợi Order Service lưu xong order
  async function waitForOrder(app, id, token, msTotal = 20000, stepMs = 1000) {
    const start = Date.now();
    await new Promise((r) => setTimeout(r, 300)); // delay nhỏ ban đầu
  ...
  describe("GET /:id", () => {
    ...
    before(async () => {
      // Tạo order để test GET /:id, ưu tiên dùng createdProductId nếu có
      createdOrderId = await createTestOrder(app, authToken, createdProductId);
    });

    it("should get order details by ID successfully", async function () {
      this.timeout(30000); // cho MQ/Order xử lý
      const res = await waitForOrder(app, createdOrderId, authToken, 20000, 1000);

      expect(res).to.have.status(200);
      expect(res.body).to.have.property("_id", createdOrderId);
      expect(res.body).to.have.property("username");
      expect(res.body).to.have.property("products");
      expect(res.body.products).to.be.an("array");
